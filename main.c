...
int keep_going = 1;			/* This variable controls the main loop. */
...
void builtin_func()
{
	printf("Funcao criada internamente.\n");
}
...
{
	if (!strcmp(cmd, "builtin_func")) {
		builtin_func();
		return 1;
	}
	if (!strcmp(cmd, "bbtchau")) {
		keep_going = 0;
		return 1;
	}
	if (!strcmp(cmd, "cd")) {
